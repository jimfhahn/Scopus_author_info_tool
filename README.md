...
4. Run author_data_extraction.py first. It will capture most articles (stored in the output.csv), but there will be a few that cannot be located by a title+year combination. Failed articles will be stored in a file called error_log.txt
...
7. Run author_data_extraction_doi.py. This time, the script will try to locate the article by doi, which is a more precise way to find an article. The output is stored in a new file called "output_doi.csv." Similar to step 4, you may capture some new articles, but there will still be failures. Failed cases are written into error_log_doi.txt.
